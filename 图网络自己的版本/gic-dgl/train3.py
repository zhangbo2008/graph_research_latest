...
from dgl import DGLGraph   # 需要装dgl库包---一个图数据的.
...
def main(args):
    ...
    if args.dataset == 'cora' or args.dataset == 'citeseer' or args.dataset == 'pubmed':
        ...
        in_feats = features.shape[1]  # 特征的维度.
        g = data.graph
        if args.dataset == 'cora':
            g.remove_edges_from(       nx.selfloop_edges(g)        ) # 删除自环.
            g.add_edges_from(      zip(g.nodes(), g.nodes())     ) # 添加所有的自环
        g = DGLGraph(g)
...
# 只保留一个大的连通分支,
    ...
    g.ndata['label'] =   torch.tensor(labels[nodes_to_keep]) # 需要都存tensor才行.
